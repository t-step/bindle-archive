...
- **Skills audited: 14** (every authored skill: `architecture-projection`,
  `context-graph`, `domi-consumer`, `fork-pr-flow`, `hands-on-keyboard`,
  `issue-work-loop`, `license-compliance-auditor`, `maintain-claude-md`,
  `package-release-integrity`, `release-captain`, `repo-hygiene-init`,
  `scoped-sequential-prs`, `session-continuity`, `verify-then-commit`), plus
...
- **Major unknowns:** most Bindle skills still have not been *invoked* by a
  real Codex session (discovery ≠ behavior); byte-level format compatibility
...
|---|---|---|---|---|---|---|---|---|---|---|
| `domi-consumer` | Detect a DomI-consumer repo and report drift status by invoking `bin/domi-status.sh`, then interpret the verdict per `docs/domi-consumer.md` | Bindle (portable contract already exists separately: [domi-consumer.md](domi-consumer.md)) | yes (frontmatter only; added after this audit's 2026-07-11 pass, not re-run against it) | installed, **tested** — pressure-tested 2026-07-14 (5 reps/arm, RED→GREEN; #107) | untested | implicit trigger; invokes `bin/domi-status.sh` (bash, provider-neutral) and reads its exit code / verdict | `bin/domi-status.sh` (portable bash detector, no Claude-only primitive), `docs/domi-consumer.md` (contract) | Claude: tested (pressure-tested 2026-07-14, #107; see PRESSURE-TESTS.md) · Codex: untested | **portable** (thin wrapper over a portable bash detector; no Claude-only primitive) | attempt a Codex invocation to verify the portable-bash-wrapper claim (Claude pressure-test done 2026-07-14, #107) |
...
| `package-release-integrity` | Validate a Python package release before publish (version-source/tag/changelog/semver-movement checks); judgment calls (change classification) return `uncertain`, never guessed; defers to DomI where a well-formed `.domi-pin` makes its `release-semver-governance` category authoritative (issue #59) | Bindle (portable contract already exists separately: [package-release-integrity.md](package-release-integrity.md)) | yes (full frontmatter + body written this task) | installed, **tested** — fixture suite passing + pressure-tested 2026-07-14 (RED→GREEN, 9 reps; see `PRESSURE-TESTS.md`); **one open FAIL recorded 2026-07-18** on the clean-release defer case (#224), maturity held at `tested` pending diagnosis | **manual, verified 2026-07-15** — Codex CLI 0.144.4 directly ran the helper against `tag-mismatch` (fail, rc 1) and `domi-governed` (defer, rc 0), matching the fixture suite and Claude-side behavior; the Claude-native skill was not installed or discovered (see `PRESSURE-TESTS.md`) | explicit invocation named in `SKILL.md`: `python3 skills/package-release-integrity/scripts/release_integrity.py check --repo <path> [--tag ...] [--prev-version ...] [--change-class ...] [--build-cmd ...] [--test-cmd ...]` (Claude-native wording, Phase 1) | Python 3 stdlib only (`tomllib`, `re`, `subprocess`); shells out to repo-supplied `--build-cmd`/`--test-cmd` (degrades to `uncertain`, never a false pass) | fixture suite (`bin/test-package-release-integrity.sh`) passing + pressure-tested 2026-07-14 (RED→GREEN, 9 reps) + Codex helper verified 2026-07-15 (tag-mismatch fail/rc 1; DomI defer/rc 0); rep counts predate the arm-declaration rule (#223) so an unknown fraction may be void | **Codex-portable** helper with a Claude-native, non-installed `SKILL.md` wrapper (Phase 1); capability status `manual` | diagnose the 2026-07-18 clean-release defer FAIL (#224) — confounded by this skill's own `domi-governed` fixture carrying an all-zeros pin sha; re-rep the arm under #223's rule |
...
| `release-captain` | Decide whether accumulated verified work should be released (evidence since latest tag → version class + timing, rationale, confidence) and, on explicit approval, drive the configured strategy's dry-run/apply to create/update the Release Please release PR; recommends + orchestrates only, never merges/tags/publishes/deploys (issue #116, L3) | Bindle (portable contract exists separately: `docs/workflows/release-captain.md`) | yes (full frontmatter + body written this task) | installed, **tested** — RED→GREEN(fail)→REFACTOR→GREEN(pass) 2026-07-15; re-test 4/4 (all triggered, none tagged), incl. a seam-present fixture; see `PRESSURE-TESTS.md` | untested — Codex/human follow the L1 contract + helpers directly; not yet independently verified | Claude-native `SKILL.md` (Phase 1); orchestrates `python3 bin/release-evidence.py` (L2) + `bin/release-strategy.sh which\|dry-run\|apply --approval-token` (L4 seam) with two human approval gates | `bin/release-evidence.py` (Python 3 stdlib), the L4 seam (`bin/release-strategy.sh` + `bin/release-strategies/local-release-please.sh`, bash) which shells to `npx release-please` at dry-run/apply time; `docs/workflows/release-captain.md` | Claude: **tested** (4/4 re-test) · Codex: untested | **portable** (contract is provider-neutral; skill is the Claude adapter; L2 helper + L4 seam are portable) | positive dry-run→gate→apply path first exercised by Bindle's own v0.5.0 cut; Codex-side verification of the contract-driven flow |
| `context-graph` | Optional authoring layer over the deterministic context-graph CLI: draft/review/confirm semantic edge proposals (`producer=skill`); never the authority for endpoint legality, candidate keys, project identity, or acceptance (issue #186, epic #140) | Bindle (deterministic authority is the CLI `bin/context-graph.py` + `bin/context_graph/`, issues #180/#183/#184/#185) | yes (full frontmatter + body written this task) | installed, **tested** — structural graduation gate `bin/test-context-graph-skill.sh` 25/25 (producer parity, illegal-combo battery, reversed-`contradicts` collapse, uncertainty invariance, no-fabricated-candidate, no-illegal-confirm) + behavioral RED→GREEN wording micro-tests + live-discovery reps 2026-07-18; see `PRESSURE-TESTS.md` | **manual, verified 2026-07-18 (#211)** — a real `codex exec` session in the `cover-story` checkout inspected the helper CLI, authored `proposal.json`, formed and ran `propose` without being handed the exact command, and received a valid candidate; an equivalent `producer=fixture` proposal emitted the same candidate key, subject key, and dependency fingerprint | Claude-native `SKILL.md` (Phase 1); drives `python3 bin/context-graph.py <verb>`; ontology/legality/identity authority all deferred to the CLI; Codex follows the portable contract/helper path directly, not a claimed installed skill | `bin/context-graph.py` + `bin/context_graph/` (Python 3 stdlib), `schemas/context-graph/v1/proposal.schema.json`; graduation gate `bin/test-context-graph-skill.sh` (+ `skills/context-graph/tests/ontology_safety.py`) | Claude: **tested** (structural gate 25/25 + behavioral RED→GREEN + live discovery) · Codex: **manual live proof** (candidate `candidate:sha256:1f80fbd3b3553a0300665e15e9ac0deafcfc4fb200b5cda635a85b070b07f65e`; fixture parity; see `PRESSURE-TESTS.md`) | **Codex-portable CLI/helper with a Claude-native SKILL.md wrapper** (the authority is provider-neutral; adapter proof is evidence-backed) | none for CLI equivalence; future work remains install/discovery behavior if Bindle decides to ship the skill itself to Codex |
| `architecture-projection` | Operator contract for the architecture-projection CLI: the `preview -> confirm -> apply` loop, the plan fingerprint round trip, how to read a plan's `note_state`/`deferred`/`over_cap`, and the generated-region rules (issue #374 slice D5d, epic #141) | Bindle (deterministic authority is the CLI `bin/architecture-projection.py` + `bin/architecture/`, issues #228/#229/#230/#374) | yes (full frontmatter + body written this task) | installed, **draft** — no pressure-test campaign yet; the CLI beneath it is covered by 741 architecture tests including a full-cycle CLI suite, but the skill's own wording has not been RED/GREEN tested | **installed, invocation-proven 2026-07-20 (#374 D5d)** — a real `codex exec --skip-git-repo-check -s workspace-write` session, given only the goal and the Bindle checkout path, read this SKILL.md and formed the entire loop itself: `init` -> `config add-binding` -> `preview` -> `confirm --fingerprint` -> `apply --approval-token`, round-tripping token `arch-plan:sha256:135965323d79ef4d02cef114c4bebedbd75d27ef08ca8115ba7c4909d49de2bf`, then verified its own work with a second `preview` reporting `note_state: current` | implicit trigger; drives `python3 bin/architecture-projection.py <verb>`; no provider-specific primitive — every verb is argv + JSON/text on stdout, and all authority (identity, legality, placement) sits in the CLI | `bin/architecture-projection.py` + `bin/architecture/` (Python 3 stdlib); a structural-graph document to consume, which this skill does not produce | Claude: **draft** (skill wording untested; CLI tested) · Codex: **live invocation proof** (real `codex exec`, graded on the filesystem — 5 files written, identity frontmatter and generated fences correct, judgments log carrying the creation-event slug — not on the session's self-report) | **shared unchanged** (the SKILL.md is provider-neutral as written; the same file installs to both providers) | pressure-test the skill's own wording before promoting `maturity` past `draft`; the Codex proof covers invocation, not wording |
