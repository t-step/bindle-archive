...
| `context-graph` | Optional authoring layer over the deterministic context-graph CLI: draft/review/confirm semantic edge proposals (`producer=skill`); never the authority for endpoint legality, candidate keys, project identity, or acceptance (issue #186, epic #140) | Bindle (deterministic authority is the CLI `bin/context-graph.py` + `bin/context_graph/`, issues #180/#183/#184/#185) | yes (full frontmatter + body written this task) | installed, **tested** — structural graduation gate `bin/test-context-graph-skill.sh` 25/25 (producer parity, illegal-combo battery, reversed-`contradicts` collapse, uncertainty invariance, no-fabricated-candidate, no-illegal-confirm) + behavioral RED→GREEN wording micro-tests 2026-07-18; see `PRESSURE-TESTS.md` | untested — a human/Codex authoring the same `proposal.json` and running the same CLI verbs gets identical results (CLI equivalence proven), but the Claude-native SKILL.md is not itself Codex-verified | Claude-native `SKILL.md` (Phase 1); drives `python3 bin/context-graph.py <verb>`; ontology/legality/identity authority all deferred to the CLI | `bin/context-graph.py` + `bin/context_graph/` (Python 3 stdlib), `schemas/context-graph/v1/proposal.schema.json`; graduation gate `bin/test-context-graph-skill.sh` (+ `skills/context-graph/tests/ontology_safety.py`) | Claude: **tested** (structural gate 25/25 + behavioral RED→GREEN) · Codex: untested (CLI equivalence proven, adapter unverified) | **portable** (the authority is the provider-neutral CLI; skill is the Claude adapter; equivalence is machine-checked) | attempt a Codex run authoring a proposal against the CLI to verify the adapter; behavioral live-discovery reps deferred to a fresh post-merge session (harness index lags a just-added skill) |
