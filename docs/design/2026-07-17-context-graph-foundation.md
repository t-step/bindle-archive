...
- reads `preview`/`candidates` output and renders it for a human;
...
- calls `bin/context-graph.py propose`/`confirm` — it never shells past the
  CLI into the package, and never bypasses the CLI to write files directly.
...
bin/context-graph.py preview --notes-home <path> --project <slug> [--repo-root <alias>=<path> ...]
...
bin/context-graph.py propose    --notes-home <path> --project <slug> --input <proposal.json>
bin/context-graph.py confirm    --notes-home <path> --project <slug> --candidate-key <key> --decision accepted|rejected [--assigned-id <id>]   # assigned-id only for identity_anchor accept

# #185 — recompute, validate, write
bin/context-graph.py apply --notes-home <path> --project <slug> [--repo-root <alias>=<path> ...]
...
bin/context-graph.py status   --notes-home <path> --project <slug>   # unresolved conflicts, stale candidates, coverage summary
```
...
`candidates` "may not conceal multiple authorities behind one verb." It
**presents the union** of #183-issued anchor candidates and #184-validated
semantic candidates; it validates or generates neither.
...
`init`, `config *` (mutating forms), `propose`, `confirm`, and `apply` acquire
the single-writer lock (§15). `config status`, `config validate`, `preview`,
`candidates`, `validate`, and `status` are read-only and never lock.
...
line 13, `candidate.schema.json` lines 40 and 48). This is a **schema-only**
patch — the native validator in `bin/context_graph/validation.py` (not yet
written; #183's dependency) must accept exactly these two kinds from the
start, so there is no runtime drift to migrate. Classify it in
...
§6 is corrected in the same PR that lands this file, and the two schema files
are patched in the same PR that implements #183 (not in this design-only
change, which touches no schema file).
...
Owned by #183 (read path) and #184 (closure-edge extraction context), thin
and read-only:
...
map.md (unanchored entry)
   ...
   --human accepts via `confirm --decision accepted --assigned-id <id>`-->
...
human-authored proposal.json  (or #186-produced)
   ...
   --valid--> validated `edge` candidate (in-memory, printed by `candidates`)
   --human accepts via `confirm --decision accepted`-->
...
- **Candidate-key construction:** exactly #180 §10's two byte-exact
  algorithms — #182 adds no third.
...
- **Conflict between multiple accepted edges:** cannot occur for the same
  ...
  does not forbid it; `status` (§13's `apply` reports it as a
  `semantic-conflict` diagnostic, informational only, never blocking apply.
  The graph is a record of human judgment, not an enforcer of judgment
  consistency.
...
2. load and reduce judgments.jsonl (#184's reducer) into effective judged
   edges and effective identity-anchor authorizations
3. parse the CURRENT map.md bytes fresh (not the preview-time bytes) —
   guarantees a first-apply anchor for an entry added since preview still
   appears in this run's final outputs
4. construct the complete planned final state in memory: nodes, edges
   (deterministic + effective judged), anchor marker insertions
5. validate the complete planned state against #180's full invariant set
   before any write occurs
6. write, in this order: (a) map.md marker insertions (atomic replace,
   minimal diff — see below), (b) index.json (atomic replace), (c)
   context.md (atomic replace)
7. release the lock
```

Step 5's whole-state validation before step 6's first write is what makes
...
### `context.md` creation vs. update vs. markerless adoption
...
- **First creation:** no `context.md` exists — apply writes it fresh from the
  ...
  HTML-comment marker convention already used for map identities).
...
- **Markerless adoption:** `context.md` exists with **no** managed-region
  ...
  still proceeds); adopting it requires an explicit one-time
  `--adopt-context-md` flag that wraps the existing content in markers as a
  single confirmed edit, never silent.

### Temporary files, atomic rename, and per-file atomicity

Every write in step 6 uses the same temp-file-in-target-directory +
`os.replace` discipline as §5. **Atomicity is honest and per-file, not
cross-file:** apply does not claim that steps 6(a)–6(c) commit as one atomic
...
`config status`/`status` (§4) detects an incomplete prior apply by comparing
`index.json`'s recorded source `dependency_fingerprint`s against a fresh
compile — a mismatch after a completed apply is impossible by construction
(step 5 validates before any write), so a mismatch found at the *next* run's
start means the previous apply was interrupted after some but not all of
step 6. Recovery is **always** a safe retry: re-running `apply` from scratch
re-derives the complete planned state from current sources (step 1–5 are
side-effect-free) and re-writes only what actually differs from what's on
disk — a map marker already written this way is semantically idempotent to
write again (same bytes), so no rollback machinery is needed. There is no
partial-apply rollback path because there is nothing to roll back to that
retry doesn't already reconstruct.
...
- **Allowed notes-home writes:** exactly the five paths in §5's layout, all
  under `<notes-home>/projects/<project-slug>/`; nothing is ever written
  above that directory or into `private-denylist.txt`.
...
### Single-writer lock (frozen, shared by #191 init/config-mutation, #184
confirm/propose, and #185 apply — per #140/#182's required-decision §10)
...
- **Acquisition:** atomic `os.open(path, O_CREAT | O_EXCL | O_WRONLY)`; on
  ...
  "operation": "init"|"config"|"propose"|"confirm"|"apply", "acquired_at":
  <ISO-8601>}` — then `fsync`.
...
| Cross-boundary endpoint-legality matrix | #183/#184 jointly | one fixture set proving direct CLI, skill-supplied proposal, human-authored proposal file, prior ledger event, and deterministic compiler edge **all** hit the same `validate_endpoint_pair` call — per #180/#182's required-decision text |
| Locking/concurrency | #191 (owns the lock's first implementation) | two-process fixture (subprocess spawn) proving exactly one of two concurrent `init` calls allocates an ID; stale-lock recovery exercised via a fixture lock file with fabricated old `acquired_at` |
| Apply safety | #185 | fixture proving semantic no-op writes nothing (mtime-comparison assertion); fixture proving a first-apply anchor added since preview appears in that same apply's output (§12 step 3) |
...
- **#184** — implements the reducer state machine (§11), `propose`/`confirm`/
  `candidates` (§4), and the endpoint-legality cross-boundary fixture set
  (§16).
- **#185** — implements `apply` (§12) including the `context.md`
  creation/update/markerless-adoption behavior and the semantic-no-op
  comparison.
...
None block a go decision. Two items are **resolved by this design** rather
than left open (§8's schema amendment, §5's file-location reading) and are
listed there, not here, because #182's body requires this section to name
only what remains genuinely open. One forward-looking item, explicitly
out of scope for a go/no-go on #191–#186:

- The `semantic-conflict` diagnostic (§11, two accepted edges expressing
  contradictory graph meaning) is informational only in v1. Whether a future
  version should surface it more prominently (e.g. a dedicated `status`
  section) is left to #186 graduation dogfood to surface as real usage
  experience, not speculated here.
...
- Every required design decision (§1 "Required design decisions" 1–15 in
  ...
  (§5), map parsing (§7), endpoint legality (§3, reusing #180), project
  identity/bindings (§6, reusing #191), compiler phases (§3), candidate-
  production authority (§3/§10), candidate/judgment lifecycle (§11), apply
  safety (§12), single-writer locking (§15), GitHub adapter (§9), projection
  bounds (§12), privacy/path enforcement (§14), provider-neutral adapter
  (§3), test matrix (§16).
...
- This design explicitly identifies what it corrects: #180's
  `design_document` enum value (§8) and the `<project>` path ambiguity in
  #182's/#191's own bodies (§5).
...
2. **`docs/design/2026-07-16-context-graph-schema.md`** §6 — same enum
   correction, folded back per that document's own update rule. Land in the
   same PR as (1).
3. **`docs/session-notes-format.md`** and **`docs/notes-home.md`** — add
...
No other upstream issue body requires a text change; every other contract
this design reuses is already consistent as reconciled.
