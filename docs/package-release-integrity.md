...
| 3 | Version-source consistency (all discovered version declarations agree) | Helper | `version_source_consistency` | `pass` if every discovered source agrees; `fail` if they disagree; `uncertain` if no source was found at all. Sources are `pyproject.toml [project]`/`[tool.poetry]`, any top-level package `__init__.py`, the `VERSION` file, and the root key of `.release-please-manifest.json`. All are peers — there is no precedence, so a `pyproject.toml` disagreeing with a `VERSION` file is a genuine inconsistency and reported as one |
...
| 5 | Changelog / release-note presence where the repo requires it | Helper | `changelog_present` | `pass` if `CHANGELOG.md` has a section for the resolved version or an `[Unreleased]` section; otherwise `fail` by default, or `uncertain` if `--no-changelog-required` was passed. When **no version resolved**, the versioned-section probe is skipped entirely and the verdict is `uncertain`, never `fail` — a check that could not run must not report failure |
