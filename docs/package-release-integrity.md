...
|---|-------|-------|-------------|--------------------------|
| 1 | Change classification (breaking / additive / patch / data-only) | **Judgment** — helper never guesses | `change_classification` | `pass` if `--change-class` names a known class; `fail` if it names an unknown one; `uncertain` if omitted — a human must classify |
