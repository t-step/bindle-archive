...
| `bin/release-publication.py` | C1/C4/C5 | C1: an open no-follow directory descriptor pins external temp evidence/upload/download cleanup; lexical replacements are left untouched and safe leaks beat deleting unknown paths. C4: exact `gh` release view/upload/download traffic plus duplicate-safe Git database API reads that bind the live remote annotated tag to the verified tag-object and commit SHAs before draft mutation and again after cleanup. C5: draft creation, asset replacement, and final publication. A created draft is strictly re-viewed and must still be unpublished with empty assets before upload. It never reads transcripts or note contents, so C3 does not apply. The tagged-release workflow is the explicit publication trigger; protected/immutable release tags are an operational prerequisite because the final API recheck and publish edit cannot be one atomic GitHub operation. Every pre-publish failure leaves the release unpublished (and any created release as a draft), and the C5 publish edit is the final process replacement after verified cleanup and the last-possible remote check. |
