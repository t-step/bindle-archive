...
| Hooks | native — Bindle ships five today under `global/hooks/` (three `PreToolUse` guards plus the two session hooks). `bin/install.sh` symlinks the scripts into `~/.claude/hooks/`; the *wiring* in `~/.claude/settings.json` is either the opt-in `bin/install-session-hooks.sh` (session hooks) or a hand edit naming that same `~/.claude/hooks/<hook>.py` path (guards). `bin/doctor.sh` reports a wired path that does not resolve, or that resolves via the checkout instead of the symlink (#264, #312) | **native primitive exists**: `hooks.json` / `[hooks]` in `config.toml`, discovered at `~/.codex/hooks.json`, `~/.codex/config.toml`, `<repo>/.codex/hooks.json`, `<repo>/.codex/config.toml`, plus plugin-bundled hooks. Different config shape and trust model (`/hooks`) than Claude's `settings.json` matcher hooks; Bindle has no Codex-hooks adapter today. Untested. |
