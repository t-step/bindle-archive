...
| Skills | native (`skills/<name>/SKILL.md` → `~/.claude/skills/<name>`) | **native primitive exists** (Codex Agent Skills: `SKILL.md` with `name`/`description` frontmatter, discovered under `.agents/skills` — repo- and user-scoped, not `~/.codex`). Bindle installs the three eligible skills (`verify-then-commit`, `fork-pr-flow` from wave 1; `architecture-projection` added for #374) via `bin/install.sh --provider codex --agents-skills-home DIR` (#57); eligibility is explicit per-skill metadata (`capabilities.json`'s `provider.codex: "installed"`), not a directory sweep. Same underlying "open agent skills standard" family as Claude's format; a real Codex session discovered both wave-1 eligible skills and, for `verify-then-commit`, read the full skill body (not just `name`/`description`) and behaviorally followed its commit gate — `fork-pr-flow` was discovered but not separately invocation-tested this wave. `architecture-projection` is invocation-tested: a real `codex exec` session read its body and drove the CLI's whole `preview`/`confirm`/`apply` token round trip without being handed a single command (see [skill-portability-audit.md](skill-portability-audit.md) uncertainty register, U1/U2). |
...
| Hooks | native — Bindle ships five today under `global/hooks/` (three `PreToolUse` guards plus the two session hooks). `bin/install.sh` symlinks the scripts into `~/.claude/hooks/`; the *wiring* in `~/.claude/settings.json` is the opt-in `bin/install-claude-hooks.sh`, which covers all five — the session hooks on a bare `install`, each guard only when named with `--guard` (#323). `bin/doctor.sh` reports a wired path that does not resolve, one that resolves via the checkout instead of the symlink (#264, #312), and a hook installed but never wired at all (#323) | **native primitive exists**: `hooks.json` / `[hooks]` in `config.toml`, discovered at `~/.codex/hooks.json`, `~/.codex/config.toml`, `<repo>/.codex/hooks.json`, `<repo>/.codex/config.toml`, plus plugin-bundled hooks. Different config shape and trust model (`/hooks`) than Claude's `settings.json` matcher hooks; Bindle has no Codex-hooks adapter today. Untested. |
