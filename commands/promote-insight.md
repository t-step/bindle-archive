...
<!-- The routing table and hard rules: claude-kit's docs/iterative-improvement.md. -->
...
1. Classify it as exactly one of (per claude-kit's
   `docs/iterative-improvement.md`):
   - **personal preference** → your kit's `global/CLAUDE.md`;
   - **reusable skill / command** → the kit's `skills/` or `commands/`;
   - **project-specific rule** → that project's `CLAUDE.md` or `.claude/`;
   ...
   promotions.
2. Draft the exact text that would land: the skill/command body, the CLAUDE.md
   lines, the profile edit, the check pattern. If the source is a private
   session note, sanitize while drafting (claude-kit's
